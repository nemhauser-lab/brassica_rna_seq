...
edinburghPath <- file.path(rootDir, "data", "Edinburgh_results")
#old path: "C:/Users/Morgan/Documents/Lab Stuff/RNA seq/Edinburgh_results"
...
fullContrasts <- makeContrasts("WT.24_vs_WT.P" = WT.24,
                               ...
                               "phyB(PSR)_vs_WT(PSR)" = 3*phyB +  phyB.S + 
                                 phyB.R -  WT.S - WT.R,
...
RvP_CommonGenes <- mergeGenes(DEGsphyB_RvP, DEGsWT_RvP) 
colnames(RvP_CommonGenes) <- gsub("\\.x", "_phyB", colnames(RvP_CommonGenes))
colnames(RvP_CommonGenes) <- gsub("\\.y", "_WT", colnames(RvP_CommonGenes))
RvP_CommonGenes <- dplyr::left_join(RvP_CommonGenes, edinburghGeneInfo, by="Gene_ID")
fname <- makeFname(folder=folder, cont="RvP_CommonGenes", FDRmax=FDRmax, logFCmin=logFCmin)
fPath <- file.path(dirPath, fname)
write.csv(RvP_CommonGenes, fPath, row.names=TRUE)

RvP_WT_Only <- DEGsWT_RvP[!(row.names(DEGsWT_RvP) %in% RvP_CommonGenes$Gene_ID), ]
save_DEGs(RvP_WT_Only, folder, "RvP_WT_only", FDRmax, logFCmin, dirPath)

RvP_phyB_Only <- DEGsphyB_RvP[!(row.names(DEGsphyB_RvP) %in% RvP_CommonGenes$Gene_ID), ]
save_DEGs(RvP_phyB_Only, folder, "RvP_phyB_only", FDRmax, logFCmin, dirPath)

plot.new()
temp <- draw.pairwise.venn(area1=nrow(RvP_WT_Only)+nrow(RvP_CommonGenes), 
                           area2=nrow(RvP_phyB_Only)+nrow(RvP_CommonGenes), 
...
allGenes_RvP$group <- rep("Neither", nrow(allGenes_RvP))
allGenes_RvP[row.names(RvP_WT_Only), ]$group <- "WT_Only"
allGenes_RvP[row.names(RvP_phyB_Only), ]$group <- "phyB_Only"
allGenes_RvP[RvP_CommonGenes$Gene_ID, ]$group <- "Both"
# allGenes_RvP[row.names(DEGs_RvP), ]$group <- "WT(R-P) vs. phyB(R-P) FDR<0.1"
allGenes_RvP_sig <- subset(allGenes_RvP, group != "Neither")

cols <- c("Neither"="grey", "WT_Only"="blue", "phyB_Only"="red", 
          "Both"="purple", "WT(R-S) vs. phyB(R-S) FDR<0.1"="green")

ggplot(allGenes_RvP, aes(x=PValue.x, y=PValue.y, color=group)) +
  geom_point(data=subset(allGenes_RvP, group=="Neither"), alpha=0.2) +
  geom_point(data=subset(allGenes_RvP, group=="Both"), alpha=0.5) +
  geom_point(data=subset(allGenes_RvP, group=="phyB_Only")) +  
  geom_point(data=subset(allGenes_RvP, group=="WT_Only")) +  
  # geom_point(data=subset(allGenes_RvP, group=="WT(R-S) vs. phyB(R-S) FDR<0.1")) +  
  scale_color_manual(values=cols) +
  scale_x_log10() + scale_y_log10() + 
  labs(x = "WT p-value", y="phyB p-value", title="p-values R vs. P")
...
ggplot(allGenes_RvP, aes(x=logFC.x, y=logFC.y, color=group)) +
  geom_point(data=subset(allGenes_RvP, group=="Neither"), alpha=0.2) +
  geom_point(data=subset(allGenes_RvP, group=="Both"), alpha=.5) +
  geom_point(data=subset(allGenes_RvP, group=="phyB_Only")) +  
  geom_point(data=subset(allGenes_RvP, group=="WT_Only")) +  
  # geom_point(data=subset(allGenes_RvP, group=="WT(R-S) vs. phyB(R-S) FDR<0.1")) +  
  scale_color_manual(values=cols) + 
  labs(x = "WT Log2 Fold Change", y="phyB Log2 Fold Change", title="Log Fold Change R vs. P")
