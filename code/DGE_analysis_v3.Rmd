...
save_DEGs <- function(DEGList, folder, cont, FDRmax, logFCmin, dir){
  ...
  fname <- makeFname(folder=paste(folder, "/promoters", sep=""), cont=cont, 
                     change="_UP", dtype="_promoters", FDRmax=FDRmax, logFCmin=logFCmin)  
  fPath <- file.path(dirPath, fname)
  writeXStringSet(allPromoters2[names(allPromoters) %in% upDEGs$Gene_ID], file.path(dirPath, fname))
  
  fname <- makeFname(folder=paste(folder, "/promoters", sep=""), cont=cont, 
                     change="_DOWN", dtype="_promoters", FDRmax=FDRmax, logFCmin=logFCmin)  
  fPath <- file.path(dirPath, fname)
  writeXStringSet(allPromoters2[names(allPromoters) %in% downDEGs$Gene_ID], file.path(dirPath, fname))
...
save_Venn_Lists <- function(allGenesList, folder, cont, FDRmax, logFCmin, change, dir){
  ...
  fname <- makeFname(folder=paste(folder, "/promoters", sep=""), 
                     cont=paste(cont, "WT_SIG", sep="_"), 
                     change=change, dtype="_promoters", FDRmax=FDRmax, logFCmin=logFCmin)
  fPath <- file.path(dirPath, fname)
  writeXStringSet(allPromoters2[names(allPromoters) %in% row.names(allGenesList[allGenesList$group == "WT_sig", ])], file.path(dirPath, fname))
  ...
  fname <- makeFname(folder=paste(folder, "/promoters", sep=""), 
                     ...
                     change=change, dtype="_promoters", FDRmax=FDRmax, logFCmin=logFCmin)
  fPath <- file.path(dirPath, fname)
  writeXStringSet(allPromoters2[names(allPromoters) %in% row.names(allGenesList[allGenesList$group == "phyB_sig", ])], file.path(dirPath, fname))
  ...
  fname <- makeFname(folder=paste(folder, "/promoters", sep=""), 
                     ...
                     change=change, dtype="_promoters", FDRmax=FDRmax, logFCmin=logFCmin)
  fPath <- file.path(dirPath, fname)
  writeXStringSet(allPromoters2[names(allPromoters) %in% row.names(allGenesList[allGenesList$group == "Both_sig", ])], file.path(dirPath, fname))  
...
# 
# # create the model matrix used for the linear model
# design <- model.matrix( ~Timepoint + Genotype:Timepoint, data=sampleCond)
# # remove column because there is no phyB plant #6
# design <- design[, !(colnames(design) %in% c("GenotypephyB:Plant6"))]
# 
# # simplify column names of design
