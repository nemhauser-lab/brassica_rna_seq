...
allGenes_DvP <- mergeGenes(allGenesWT_DvP, allGenesphyB_DvP)
...
```{r}
# --- DOWN --- #
group <- plyr::aaply(.data=allGenes_DvP, .margins=1, .fun=setGroup, change="down",
                       sigLvl=FDRmax, LFCutoff=logFCmin, .expand=FALSE)
allGenes_DvP$group <- group
...
allGenes_RvD <- mergeGenes(allGenesWT_RvD, allGenesphyB_RvD)
...
# --- UP --- #
group <- plyr::aaply(.data=allGenes_RvD, .margins=1, .fun=setGroup, change="up",
                       sigLvl=FDRmax, LFCutoff=logFCmin, .expand=FALSE)
allGenes_RvD$group <- group
...
```{r}
# --- DOWN --- #
group <- plyr::aaply(.data=allGenes_RvD, .margins=1, .fun=setGroup, change="down",
                       sigLvl=FDRmax, LFCutoff=logFCmin, .expand=FALSE)
allGenes_RvD$group <- group
